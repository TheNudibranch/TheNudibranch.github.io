...
These deviations reflect years of practical refinement and are necessary to match the `loo` package's output. The Appendix gives the clean theory; the code below gives the battle-tested version.
:::
...
Let's tie everything together with a proper example. We'll fit a Bayesian linear regression using `rstanarm` with the same data from the very beginning of the post. The beauty of `rstanarm` is that it comes with pre-compiled Stan models, so there is no compilation step. We get straight to sampling.
...
fit <- stan_glm(y ~ x, data = data,
                ...
                chains = 4, iter = 2000, refresh = 0, seed = 123)
```

Under the hood, `rstanarm` is fitting the same model we would write in raw Stan. The difference is that the log-likelihood for every observation is computed automatically for us. We can extract it with the `log_lik` method and use it directly for our importance ratios.

Now let's extract the log-likelihood matrix and compute PSIS-LOO both manually and with the `loo` package:

```{r}
#| eval: false
log_lik_mat <- log_lik(fit)
...
Our manual estimates should track closely to the `loo` package results.^[They won't be exactly identical because the `loo` package uses a more refined implementation of the Zhang and Stephens estimator, but the spirit is the same.] The gold diagonal line shows perfect agreement.
