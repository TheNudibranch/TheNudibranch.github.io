...
```{r, fig.align='center', set.seed(1234)}
...
posterior_oxy <- lapply(simulation, function(x) return(x[-seq(1,burn),1])) %>% 
  unlist() %>% unname()
plot(posterior_oxy %>% density(), lwd = 4, xlab='', ylab='', main='Posterior for Dissolved Oxygen Paramter')
...
posterior_depth <- lapply(simulation, function(x) return(x[-seq(1,burn),2])) %>% 
  unlist() %>% unname()
plot(posterior_depth %>% density(), lwd = 4, xlab='', ylab='', main='Posterior for Depth Paramter')
...
colnames(metric_mat) <- c('b_oxy', 'b_depth', 'b_int')
...
hist(posterior_predictive,prob=T, col=adjustcolor('darkblue', alpha.f = 0.5), breaks=length(unique(posterior_predictive)),
     main='', ylab='', )
